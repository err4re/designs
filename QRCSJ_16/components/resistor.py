...
class ResParams:
    ...
    arm_bottom: float = 0.8
    arm_width: float = 0.8
    num_segments: int = 32

    min_spacing = 3 #minimal spacing for arms on small resistor
    ...
    opposite_side_connectors: bool = False

    mirrored_x_axis: bool = False

    symmetric: bool = False # useful for 4-wire setups
    symmetric_shift: float = 0.7

    ebeam_layer: Layer = default_ls['ebeam']
    undercut_layer: Layer = default_ls['undercut']
    highdose_layer: Layer = default_ls['ebeam_junctions']
...
class Resistor:
    ...
    def create_arm(res_params: ResParams) -> Tuple[Device, Device]:
        ...
        # add extra box on the bottom, to ensure contact between first Al layer and Cr everywhere
        Arm << pg.rectangle(size=(res_params.arm_width, -res_params.arm_bottom), layer=res_params.ebeam_layer)

        # add symmetric box on other side for symmetric resistor
        if res_params.symmetric:
            Arm << pg.rectangle(size=(res_params.arm_width, res_params.arm_bottom), layer=res_params.ebeam_layer).movey(height)

            # add undercut for this side
            Undercut_Box << pg.rectangle(size=(res_params.arm_width + res_params.undercut_width, res_params.arm_bottom + res_params.undercut_width)).move((- res_params.undercut_width, height))
    ...
    def create_left_arms(self, res_params: ResParams) -> Tuple[list[Device], list[Device]]:
        ...
        if res_params.small_resistor:
            ...
            if res_params.mirrored_x_axis:
                arm = Arm + Undercut
                arm.movex(res_params.segment_length + res_params.arm_width)
        ...
        # for big resistors all resistive segments are in series
            ...
            if res_params.mirrored_x_axis:
                num_left_arms = res_params.num_segments // 2
            else:
                num_left_arms = res_params.num_segments // 2 - 1 + res_params.num_segments%2

            for i in range(num_left_arms):
                Arm, Undercut = Resistor.create_arm(res_params)
    ...
    def create_right_arm_small(res_params: ResParams) -> Tuple[Device, Device]:
        ...
        #                       res_params.total_spacing + max(res_params.min_spacing, res_params.spacing) + (res_params.num_segments)*res_params.resistor_width + res_params.connector_height/2),
        #                  layer=res_params.ebeam_layer)
        
        if res_params.opposite_side_connectors:
            Arm, Undercut = Resistor.create_arm(res_params)

            return Arm, Undercut

        else:
            Lr = pg.L(width=res_params.connector_height, 
                    size=(res_params.segment_length + res_params.connector_height/2 + res_params.arm_width,
                                res_params.total_spacing + max(res_params.min_spacing, res_params.spacing) + (res_params.num_segments)*res_params.resistor_width + res_params.connector_height/2),
                    layer=res_params.ebeam_layer)
            
        Offset = pg.rectangle(size=(res_params.connector_height - res_params.arm_width, Lr.ysize))
        Offset.move(origin=(Offset.xmin, Offset.ymin), destination=(Lr.xmin, Lr.ymin))

        L = Arm << pg.boolean(Lr, Offset, operation='A-B', layer=res_params.ebeam_layer)

        
        Al_Cr_Box =  pg.rectangle(size=(res_params.arm_width, res_params.arm_bottom), layer=res_params.ebeam_layer)
        # Al_Cr_Box.move((-res_params.connector_height/2, res_params.total_spacing + max(res_params.min_spacing, res_params.spacing) + (res_params.num_segments)*res_params.resistor_width + res_params.connector_height/2))
        
        Al_Cr_Box.move(origin=(Al_Cr_Box.xmin, Al_Cr_Box.ymin), destination=(Arm.xmin, Arm.ymax))

        Arm << Al_Cr_Box

        Arm.movey(res_params.arm_bottom)
        ...
        if res_params.mirrored_x_axis:
            Undercut_Box.movex(-res_params.undercut_width)
    ...
    @staticmethod
    def create_right_arm_small_mirrored(res_params: ResParams) -> Tuple[Device, Device]:
        Arm = Device('Right arm connector')
        ...
        Al_Cr_Box =  pg.rectangle(size=(res_params.arm_width, res_params.arm_bottom), layer=res_params.ebeam_layer)
        # Al_Cr_Box.move((-res_params.connector_height/2, res_params.total_spacing + max(res_params.min_spacing, res_params.spacing) + (res_params.num_segments)*res_params.resistor_width + res_params.connector_height/2))
        Al_Cr_Box.move(origin=(Al_Cr_Box.ymin, Al_Cr_Box.xmin), destination=(Arm.xmin, Arm.ymax))
        Arm << Al_Cr_Box
    ...
    def create_right_arms(self, res_params: ResParams) -> Tuple[list[Device], list[Device]]:
        ...
        if res_params.small_resistor:
            ...
            Right_Arm.move(origin=(Arm.xmax - res_params.arm_width, Arm.ymax - res_params.arm_bottom), destination=(self.res_segments.xmax, self.res_segments.ymax))
        ...
        # for big resistors all resistive segments are in series
            ...
            if res_params.mirrored_x_axis:
                num_right_arms = res_params.num_segments // 2 - 1 + res_params.num_segments%2
            else:
                num_right_arms = res_params.num_segments // 2

            for i in range(num_right_arms):
                Arm, Undercut = Resistor.create_arm(res_params)
    ...
    def create_arms(self, res_params: ResParams) -> Tuple[list[Device], list[Device]]:
        ...
        if res_params.mirrored_x_axis and not res_params.small_resistor:
            [arm.movey(res_params.spacing + res_params.resistor_width) for arm in left_arms]
            [undercut.movey(res_params.spacing + res_params.resistor_width) for undercut in undercuts]

        # subtract resistive segements from undercuts to avoid overlap
        undercuts = [pg.boolean(A=undercut, B=pg.offset(elements=self.res_segments, distance=res_params.undercut_spacing), operation='A-B', layer=res_params.undercut_layer) for undercut in undercuts]
        ...
        if res_params.mirrored_x_axis and not res_params.small_resistor:
            [arm.movey(- res_params.spacing - res_params.resistor_width) for arm in right_arms]
            [undercut.movey( - res_params.spacing - res_params.resistor_width) for undercut in undercuts]
    ...
    def create_connectors(self, res_params: ResParams) -> Tuple[Device, Device, Device]:
        ...
        Top_Undercut = Device('Top Undercut')
        Bot_Undercut = Device('Bot Undercut')
        ...
        # if not res_params.mirrored_x_axis:
        Top_Connector << pg.rectangle(size=(-res_params.connector_width, -res_params.arm_bottom), layer=res_params.ebeam_layer)
        

        Undercut_Box = pg.rectangle(size=(-Top_Connector.xsize - res_params.undercut_width, Top_Connector.ysize +2*res_params.undercut_width), layer=res_params.undercut_layer)
        Undercut_Box.movey(-res_params.arm_bottom-res_params.undercut_width)


        Top_Undercut << pg.boolean(A=Undercut_Box, B=pg.offset([Top_Connector], distance=res_params.undercut_spacing), operation='A-B', layer=res_params.undercut_layer)

        top_connector = Top_Connector + Top_Undercut

        if res_params.mirrored_x_axis:
                Top_Connector.mirror()
                Top_Undercut.movex(res_params.connector_width)
                top_connector.movex(res_params.segment_length)



        # odd number of resistive segments?
        if res_params.num_segments%2 and not res_params.small_resistor:
            connector, undercut = Resistor.create_connector(res_params)

            if res_params.symmetric:

                connector << pg.rectangle(size=(-res_params.connector_width, res_params.arm_bottom), layer=res_params.ebeam_layer).movey(res_params.connector_height)
                undercut << pg.rectangle(size=(-connector.xsize - res_params.undercut_width, connector.ysize + res_params.undercut_width), layer=res_params.undercut_layer)
            ...
            # if res_params.mirrored_x_axis:
            #     connector << pg.rectangle(size=(-res_params.connector_width, -res_params.arm_bottom), layer=res_params.ebeam_layer)
            ...
            if res_params.small_resistor:
                ...
                bot_connector.move(origin=(connector.xmin, connector.ymax), destination=(self.res_segments.xmax, self.res_segments.ymin + res_params.resistor_width + int(res_params.symmetric)*res_params.arm_bottom))

            
            if res_params.mirrored_x_axis:
                undercut.movex(- res_params.connector_width- res_params.segment_length)
                connector.movex(- res_params.segment_length)
                connector.mirror()
                

            # subtract resistive segements from undercuts to avoid overlap
            undercut = pg.boolean(A=undercut, B=pg.offset(elements=[self.res_segments, connector], distance=res_params.undercut_spacing), operation='A-B', layer=res_params.undercut_layer)


            Bot_Connector << connector
            Bot_Undercut << undercut

        # even number of resistive segments or small resistor
        else:
            connector, undercut = Resistor.create_connector(res_params)
            ...
            if res_params.small_resistor and res_params.opposite_side_connectors:
                bot_connector.move(origin=(bot_connector.xmax, bot_connector.y), destination=(self.res_arms.xmax + res_params.connector_width - res_params.arm_width, self.res_arms.ymin+res_params.connector_height/2))
            elif res_params.small_resistor:
                bot_connector.move(origin=(bot_connector.xmax, bot_connector.y), destination=(self.res_arms.xmin+res_params.arm_width, self.res_arms.ymin+res_params.connector_height/2))
            else:    
            ...
            if res_params.mirrored_x_axis:
                connector.mirror()
                undercut.movex(res_params.connector_width)
                bot_connector.movex(res_params.segment_length)
        ...
        #     Top_Connector.mirror(p1=(0,Top_Connector.center[1]), p2=(1,Top_Connector.center[1]))
        #     top_connector = Top_Connector + Top_Undercut
        #     bot_connector = Bot_Connector + Bot_Undercut

        #     top_connector_bot_left_corner = (top_connector.xmin, top_connector.ymin)
        #     top_connector_top_left_corner = (top_connector.xmin, top_connector.ymax)
        #     bot_connector_bot_left_corner = (bot_connector.xmin, bot_connector.ymin)
        #     bot_connector_top_left_corner = (bot_connector.xmin, bot_connector.ymax)

        #     top_connector.move(origin = top_connector_bot_left_corner, destination= bot_connector_bot_left_corner)
        #     bot_connector.move(origin = bot_connector_top_left_corner, destination= top_connector_top_left_corner)


        #     Undercut << [Top_Undercut, Bot_Undercut]
        #     Undercut = pg.boolean(A=Undercut, B=pg.offset([self.res_segments], distance=res_params.undercut_spacing), operation='A-B', layer=res_params.undercut_layer)

        #     # inverted order due to mirrored x-axis!
        #     return Bot_Connector, Top_Connector, Undercut
    ...
    def add_ports(self, res_params: ResParams) -> None:
        ...
        if res_params.connectors:
            ...
            # else:
            self.device.add_port(name='top', midpoint=(self.top_connector.xmin, (self.top_connector.ymax + self.top_connector.ymin)/2 + res_params.arm_bottom/2), width=res_params.connector_height, orientation=180)
        ...
        if res_params.small_resistor:
            ...
            if res_params.mirrored_x_axis:
                self.device.add_port(name='top left small', midpoint=(self.res_arms.xmin+res_params.arm_width/2 + res_params.segment_length + res_params.arm_width, self.res_arms.ymax -res_params.arm_bottom), width=res_params.arm_width, orientation=90)
                self.device.add_port(name='bot left small', midpoint=(self.res_arms.xmin+res_params.arm_width/2 + res_params.segment_length + res_params.arm_width, self.res_arms.ymin), width=res_params.arm_width, orientation=-90)
            else:
                self.device.add_port(name='top left small', midpoint=(self.res_arms.xmin+res_params.arm_width/2, self.res_arms.ymax -res_params.arm_bottom), width=res_params.arm_width, orientation=90)
            ...
            if res_params.connectors and res_params.opposite_side_connectors:
            ...
            if res_params.num_segments%2:
                if res_params.connectors:
                    if res_params.symmetric:
                        self.device.add_port(name='bot', midpoint=(self.bot_connector.xmax, (self.bot_connector.ymax + self.bot_connector.ymin)/2 - res_params.arm_bottom/2), width=res_params.connector_height, orientation=0)
                    else:
                        self.device.add_port(name='bot', midpoint=(self.bot_connector.xmax, (self.bot_connector.ymax + self.bot_connector.ymin)/2), width=res_params.connector_height, orientation=0)
