...
class JJ4Wire:
    ...
    @staticmethod
    def create_top_small_resistors(res_params: ResParams) -> Tuple[Device, Device]:

        TopLeftResistor = Device('Left resistor')
        TopRightResistor = Device('Right resistor')

        left_resistor = Resistor()
        left_resistor.generate_resistor(res_params)

        TopLeftResistor << left_resistor.device

        right_resistor = Resistor()
        right_res_params = deepcopy(res_params)
        right_res_params.mirrored_x_axis = True
        right_resistor.generate_resistor(right_res_params)

        TopRightResistor << right_resistor.device

        [TopLeftResistor.add_port(port) for port in left_resistor.device.get_ports()]
        [TopRightResistor.add_port(port) for port in right_resistor.device.get_ports()]
    ...
    @staticmethod
    def create_top_small_resistors_opposite_side_connectors(res_params: ResParams) -> Tuple[Device, Device]:
    ...
    @staticmethod
    def create_bot_small_resistors(res_params: ResParams) -> Tuple[Device, Device]:
    ...
    def create_small_resistors(self, res_params: ResParams) -> None:

        TopLeftResistor, TopRightResistor = JJ4Wire.create_top_small_resistors(res_params)
        BotLeftResistor, BotRightResistor = JJ4Wire.create_bot_small_resistors(res_params)
    ...
    def position_small_resistors(self) -> None:
        top_resistors: Group = self.top_left_resistor + self.top_right_resistor
        top_resistors.move(origin=self.top_left_resistor.ports['bot'], destination=self.junction_ref.ports['top'])
