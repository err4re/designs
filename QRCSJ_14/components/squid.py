...
class SquidParams(JJParams):
    ...
    def to_jj_params(self) -> JJParams:
        ...
        return JJParams(
            ...
            stress_layer=self.stress_layer
        )
...
class Squid:
    ...
    def __init__(self) -> None:
        ...
        self.short: Optional[DeviceReference] = None
    ...
    @staticmethod
    def create_left_short(squid_params: SquidParams) -> Device:

        Short = Device('Short')

        #top and bottom connector
        Short << pg.rectangle(size=(squid_params.short_distance_from_squid, squid_params.island_height), layer=squid_params.layer)
        Short << pg.rectangle(size=(squid_params.short_distance_from_squid, squid_params.island_height), layer=squid_params.layer).movey(-squid_params.loop_height-squid_params.island_height)

        #arm (to be scratched away)
        Short << pg.rectangle(size=(squid_params.arm_width, squid_params.loop_height), layer=squid_params.layer).movey(-squid_params.loop_height)

        Short.add_port(name='top', midpoint=((squid_params.short_distance_from_squid, squid_params.island_height/2)), width=squid_params.island_height, orientation=0)
        Short.add_port(name='bot', midpoint=((squid_params.short_distance_from_squid, -squid_params.island_height/2 - squid_params.loop_height)), width=squid_params.island_height, orientation=0)         
        
        return Short
    
    # def add_left_short(squid_params: SquidParams) -> None:
    ...
    # def add_right_short(squid_params: SquidParams) -> None:
    ...
    def create_liftoff_undercut(self, squid_params: SquidParams) -> Device:
        if (self.device is None):
            ...
            Top_Undercut = pg.rectangle(size=(int(squid_params.shorted)*squid_params.short_distance_from_squid + squid_params.island_width+squid_params.arm_width + squid_params.undercut_width, squid_params.island_height + 2*squid_params.undercut_width), layer=squid_params.undercut_layer)
            Top_Undercut.move((-squid_params.arm_width/2 - squid_params.undercut_width - (int(squid_params.short_on_left_side)) *int(squid_params.shorted)*squid_params.short_distance_from_squid, squid_params.loop_height/2 - squid_params.undercut_width))
        
            Bot_Undercut = pg.rectangle(size=(int(squid_params.shorted)*squid_params.short_distance_from_squid + squid_params.island_width+squid_params.arm_width + squid_params.undercut_width, squid_params.island_height + 2*squid_params.undercut_width), layer=squid_params.undercut_layer)
            Bot_Undercut.move((-squid_params.arm_width/2 - squid_params.undercut_width - (int(squid_params.short_on_left_side)) *int(squid_params.shorted)*squid_params.short_distance_from_squid, -(squid_params.loop_height/2 + squid_params.island_height) - squid_params.undercut_width))
    ...
    def generate_squid(self, squid_params: SquidParams, overwrite: bool = False) -> None:
        if (self.device is not None) and not overwrite:
            ...
            if squid_params.shorted:
                ...
                self.short = self.device << Squid.create_left_short(squid_params)

                if squid_params.short_on_left_side:

                    # put short in correct position
                    self.short.connect(self.short.ports['top'], destination=self.device.ports['top left'])        

                else:

                    # put short in correct position
                    self.short.connect(self.short.ports['bot'], destination=self.device.ports['top right'])
