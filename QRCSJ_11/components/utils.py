...
class WritefieldParams:
    writefield_width: float = 96.0
    writefield_height: float = 96.0
...
def write_position_list(file_name: str, gds_path: str,
                        ...
                        exposition_layers: Union[list[Layer], tuple[int]],
    ...
    with open(f'position_lists/{file_name}.pls', 'a') as f:
        ...
        for i, (writefield_rect, working_area_rect) in enumerate(zip(writefield_corners, working_area_corners)):
            ...
            f.write(str(float(writefield_width)) + '00,' + str(float(writefield_height)) + '00,')
            f.write(',,,,')

            #half of writefield size
            f.write(str(float(writefield_width)/2) + '00,' + str(float(writefield_height)/2) + '00,')
            f.write(',,')
            
            f.write(gds_path) # f.write('D:\Alex\Ebeam litho\QRCSJ_R7\Run 2\QRCSJ_R7_v2.gds')

            #layers to be exposed
            if isinstance(exposition_layers, list):
            ...
            elif isinstance(exposition_layers, tuple):
                f.write(',' + str(exposition_layers[0]) + '-' + str(exposition_layers[1]) + ',') # f.write(',1-3,') exposition layers all from initial to final index....
            else:
                raise Exception('ERROR: Exposition layers has to be either list or tuple!')
