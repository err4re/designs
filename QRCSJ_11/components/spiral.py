...
class Spiral:
    ...
    def create_connector(self, spiral_params: SpiralParams) -> Device:
        ...
        Compass = Connector << pg.compass(size=(spiral_params.connector_length + 2*extra_length, spiral_params.w), layer= spiral_params.layer)
        for port in Compass.ports.values():
            Connector.add_port(port)

        
        Connector.move(origin=Connector.ports['E'], destination=(self.device.xmin + 2*extra_length , spiral_params.connector_shift))

        Conn = pg.boolean(A=Connector, B=pg.circle(radius=self.device.xmin + spiral_params.w/2), operation='A-B', layer=spiral_params.layer)
        for port in Compass.ports.values():
            Conn.add_port(port)
    ...
    def add_ports(self, spiral_params: SpiralParams) -> None:
        ...
        if spiral_params.connector_length > 0:
            ...
            self.device.add_port(name='capa bot', port=self.connector.ports['W'])
